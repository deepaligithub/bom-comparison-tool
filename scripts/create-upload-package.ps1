<#
.SYNOPSIS
  Creates a deployment package (UploadPackage) with the app and all documentation, ready to upload or distribute.
.DESCRIPTION
  Builds the frontend, copies backend (with embedded static build), all docs, samples, and run scripts into a single folder.
  Optionally creates a zip file.
.EXAMPLE
  .\scripts\create-upload-package.ps1
  .\scripts\create-upload-package.ps1 -CreateZip
#>
Param(
  [string]$PackageName = "BOMCompareTool-Package",
  [switch]$CreateZip,
...
Write-Host "=== BOM Compare Tool - Create Upload Package ===" -ForegroundColor Cyan
...
# 10) Package readme (how to run)
$hasExe = Test-Path (Join-Path $PackageDir "BOMCompareTool.exe")
$packageReadme = @"
BOM Compare Tool - Upload Package
=================================
...
if ($CreateZip) {
  ...
  Write-Host "DONE: Package at $PackageDir and zip at $zipPath" -ForegroundColor Green
} else {
  Write-Host "DONE: Package at $PackageDir" -ForegroundColor Green
}
